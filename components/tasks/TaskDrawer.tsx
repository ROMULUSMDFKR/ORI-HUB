    ...
    return (
        <div ref={dropdownRef} className="relative">
            ...
            <div className="mt-1">
                ...
                {isOpen && (
                    <div className="border bg-surface rounded-md shadow-lg mt-1 absolute z-10 w-full max-w-sm">
                            ...
                            {filteredUsers.map(user => (
                                <li key={user.id}>
                                    <Checkbox
                                        ...
                                        className="p-2 hover:bg-background w-full"
                                    >
            ...
            <div className="flex flex-col h-full">
                <div className="flex-1 overflow-y-auto pr-2 -mr-6 pl-1 -ml-6 py-1">
                    <div className="space-y-6 px-6">
                        ...
                        <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-on-surface-secondary">Estado</label><select value={editedTask.status || ''} onChange={e => handleFieldChange('status', e.target.value as TaskStatus)} className={sharedInputClass}>{Object.values(TaskStatus).map(s => <option key={s} value={s}>{s}</option>)}</select></div><div><label className="block text-sm font-medium text-on-surface-secondary">Prioridad</label><select value={editedTask.priority || ''} onChange={e => handleFieldChange('priority', e.target.value as Priority)} className={sharedInputClass}>{Object.values(Priority).map(p => <option key={p} value={p}>{p}</option>)}</select></div></div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="block text-sm font-medium text-on-surface-secondary">Fecha de Inicio</label><input type="date" value={editedTask.startDate?.split('T')[0] || ''} onChange={e => handleFieldChange('startDate', e.target.value ? new Date(e.target.value).toISOString() : undefined)} className={sharedInputClass}/></div>
                            <div><label className="block text-sm font-medium text-on-surface-secondary">Fecha de Vencimiento</label><input type="date" value={editedTask.dueAt?.split('T')[0] || ''} onChange={e => handleFieldChange('dueAt', e.target.value ? new Date(e.target.value).toISOString() : undefined)} className={sharedInputClass}/></div>
                        </div>

                        <div><label className="block text-sm font-medium text-on-surface-secondary">Proyecto</label><select value={editedTask.projectId || ''} onChange={e => handleFieldChange('projectId', e.target.value)} className={sharedInputClass}><option value="">Ninguno</option>{MOCK_PROJECTS.map((p: Project) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
                        
                        <div><label className="block text-sm font-medium text-on-surface-secondary">Etiquetas</label><div className="flex flex-wrap gap-2 mt-1">{editedTask.tags?.map(tag => <div key={tag} className="flex items-center gap-1 bg-gray-200 rounded-full pl-2 pr-1 text-sm"><Badge text={tag} /><button onClick={() => handleRemoveTag(tag)} className="text-gray-500 hover:text-gray-800">&times;</button></div>)}</div><input type="text" value={newTag} onChange={e => setNewTag(e.target.value)} onKeyDown={handleAddTag} placeholder="Añade una etiqueta y presiona Enter" className={`${sharedInputClass} mt-2`} /></div>
                        ...
                        <div className="border-t pt-4">
                            ...
                            <div className="mt-2 flex gap-2"><input type="text" placeholder="Añadir sub-tarea..." value={newSubtask} onChange={e => setNewSubtask(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddSubtask()} className={`${sharedInputClass} flex-1`} /><button onClick={handleAddSubtask} className="bg-primary text-on-primary font-semibold px-3 rounded-lg shadow-sm">Añadir</button></div>
                        </div>

                        {task?.id && (
                            <div className="border-t pt-4 relative">
                                <h4 className="text-sm font-semibold text-on-surface-secondary mb-2">Comentarios</h4>
                                <div className="space-y-3 max-h-60 overflow-y-auto pr-2">{editedTask.comments?.map(comment => { const user = Object.values(MOCK_USERS).find(u => u.id === comment.userId); return (<div key={comment.id} className="flex items-start gap-2"><img src={user?.avatarUrl} alt={user?.name} className="w-6 h-6 rounded-full mt-1" /><div className="flex-1 bg-surface-inset p-2 rounded-lg"><p className="text-xs font-semibold">{user?.name} <span className="font-normal text-on-surface-secondary ml-2">{new Date(comment.createdAt).toLocaleString()}</span></p><p className="text-sm mt-1">{comment.text}</p></div></div>);})}</div>
                                {showUserSuggestions && userSuggestions.length > 0 && (<div className="absolute bottom-20 mb-2 w-full bg-surface border rounded-md shadow-lg z-10 max-h-40 overflow-y-auto"><ul>{userSuggestions.map(user => (<li key={user.id} onClick={() => handleMentionSelect(user.name)} className="flex items-center gap-2 p-2 hover:bg-background cursor-pointer"><img src={user.avatarUrl} alt={user.name} className="w-6 h-6 rounded-full" /><span className="text-sm">{user.name}</span></li>))}</ul></div>)}
                                <div className="mt-4 flex gap-2"><textarea ref={commentInputRef} placeholder="Añadir un comentario... (@ para mencionar)" value={newComment} onChange={handleCommentChange} rows={2} className={`${sharedInputClass} flex-1`}/><button onClick={handleAddComment} className="bg-primary text-on-primary font-semibold px-3 rounded-lg shadow-sm self-end h-10">Enviar</button></div>
                            </div>
                        )}
                    </div>
                </div>
                <div className="p-4 bg-background border-t border-border flex justify-end gap-2 -mx-6 -mb-6 mt-6"><button onClick={onClose} className="bg-surface border border-border text-on-surface font-semibold py-2 px-4 rounded-lg shadow-sm">Cancelar</button><button onClick={handleSave} className="bg-accent text-on-dark font-semibold py-2 px-4 rounded-lg shadow-sm">{task?.id ? 'Guardar Cambios' : 'Crear Tarea'}</button></div>
            </div>
        </Drawer>
    );
};
