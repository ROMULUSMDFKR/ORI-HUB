...
const EditTaskPage: React.FC = () => {
    ...
    useEffect(() => {
        if (initialTask) {
            setTask({
                ...initialTask,
                impact: initialTask.impact || 'Medio',
                completionPercentage: initialTask.completionPercentage || 0,
                actualHours: initialTask.actualHours || 0,
                isRecurring: initialTask.isRecurring || false,
                recurrenceFrequency: initialTask.recurrenceFrequency || 'Semanal',
                tags: initialTask.tags || [],
                links: initialTask.links || {}
            });
            if (initialTask.links?.blockedBy) {
                setBlockingTaskId(initialTask.links.blockedBy);
            }
        }
    }, [initialTask]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await api.updateDoc('tasks', id, updateData);
            showToast('success', 'Tarea actualizada correctamente.');
            navigate(`/tasks/${id}`);
        } catch (error) {
            console.error("Error updating task:", error);
            showToast('error', "Hubo un error al actualizar la tarea.");
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const taskOptions = [{value: '', name: 'Ninguna'}, ...(allTasks || []).filter(t => t.id !== id).map(t => ({ value: t.id, name: t.title }))];
                ...
                <div className="lg:col-span-8 space-y-6">
                    <FormCard title="Información Básica" icon="description">
                        <div>
                            ...
                            <input type="text" value={task.title} onChange={e => handleFieldChange('title', e.target.value)} className={`w-full text-lg font-medium ${baseInputClasses}`} />
                        </div>
                        <div>
