...
const TaskDetailPage: React.FC = () => {
    ...
    const handleSave = () => {
        ...
        alert(`Tarea "${task.title}" guardada (simulación).`);
        navigate('/tasks');
    };
    ...
    const handleRemoveTag = (tagToRemove: string) => {
        if (task) handleFieldChange('tags', (task.tags || []).filter(tag => tag !== tagToRemove));
    };
    ...
    const handleAddComment = () => {
        ...
        handleFieldChange('comments', [...(task.comments || []), comment]);
        setNewComment('');
    };
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-on-surface">Detalles de la Tarea</h1>
                <div className="flex gap-2">
                    <button onClick={() => navigate(-1)} className="bg-surface border border-border text-on-surface font-semibold py-2 px-4 rounded-lg shadow-sm">Volver</button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Left Column: Details */}
                <div className="lg:col-span-2 bg-surface p-6 rounded-xl shadow-sm border border-border space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         <div className="space-y-6">
                            <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-on-surface-secondary">Estado</label><select value={task.status || ''} onChange={e => handleFieldChange('status', e.target.value as TaskStatus)} className={sharedInputClass}>{Object.values(TaskStatus).map(s => <option key={s} value={s}>{s}</option>)}</select></div><div><label className="block text-sm font-medium text-on-surface-secondary">Prioridad</label><select value={task.priority || ''} onChange={e => handleFieldChange('priority', e.target.value as Priority)} className={sharedInputClass}>{Object.values(Priority).map(p => <option key={p} value={p}>{p}</option>)}</select></div></div>
                        ...
                        <div className="space-y-6">
                             <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-on-surface-secondary">Fecha de Inicio</label><input type="date" value={task.startDate?.split('T')[0] || ''} onChange={e => handleFieldChange('startDate', e.target.value ? new Date(e.target.value).toISOString() : undefined)} className={sharedInputClass}/></div><div><label className="block text-sm font-medium text-on-surface-secondary">Fecha de Vencimiento</label><input type="date" value={task.dueAt?.split('T')[0] || ''} onChange={e => handleFieldChange('dueAt', e.target.value ? new Date(e.target.value).toISOString() : undefined)} className={sharedInputClass}/></div></div>
                    ...
                    <div className="border-t pt-4">
                        ...
                        <div className="space-y-2">{task.subtasks?.map(st => (
                            <div key={st.id}>
                                <Checkbox id={`detail-subtask-${st.id}`} checked={st.isCompleted} onChange={() => handleToggleSubtask(st.id)}>
                ...
                <div className="lg:col-span-1 bg-surface p-6 rounded-xl shadow-sm border border-border flex flex-col">
                    <h3 className="text-lg font-semibold text-on-surface mb-4">Comentarios</h3>
                    <div className="flex-1 space-y-4 overflow-y-auto pr-2 -mr-2 mb-4">
                        {task.comments?.map(comment => { 
                            ...
                            return (<div key={comment.id} className="flex items-start gap-3"><img src={user?.avatarUrl} alt={user?.name} className="w-8 h-8 rounded-full mt-1" /><div className="flex-1 bg-surface-inset p-3 rounded-lg"><p className="text-xs font-semibold">{user?.name} <span className="font-normal text-on-surface-secondary ml-2">{new Date(comment.createdAt).toLocaleString()}</span></p><p className="text-sm mt-1">{comment.text}</p></div></div>);
                        })}
                        {(!task.comments || task.comments.length === 0) && <p className="text-sm text-center text-on-surface-secondary py-8">No hay comentarios aún.</p>}
                    </div>
                    <div className="relative mt-auto">
                        {showUserSuggestions && userSuggestions.length > 0 && (<div className="absolute bottom-full mb-2 w-full bg-surface border rounded-md shadow-lg z-10 max-h-40 overflow-y-auto"><ul>{userSuggestions.map(user => (<li key={user.id} onClick={() => handleMentionSelect(user.name)} className="flex items-center gap-2 p-2 hover:bg-background cursor-pointer"><img src={user.avatarUrl} alt={user.name} className="w-6 h-6 rounded-full" /><span className="text-sm">{user.name}</span></li>))}</ul></div>)}
                        <textarea ref={commentInputRef} placeholder="Añadir un comentario... (@ para mencionar)" value={newComment} onChange={handleCommentChange} rows={3} className={`${sharedInputClass}`}/>
                        <button onClick={handleAddComment} className="bg-primary text-on-primary font-semibold px-4 py-2 rounded-lg shadow-sm mt-2 float-right">Enviar</button>
                    </div>
                </div>
            </div>
        </div>
