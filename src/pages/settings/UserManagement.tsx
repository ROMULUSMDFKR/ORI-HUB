...
const CreateUserDrawer: React.FC<{ isOpen: boolean; onClose: () => void; teams: Team[]; companies: Company[]; roles: Role[]; onInvite: (newUser: any) => void }> = ({ isOpen, onClose, teams, companies, roles, onInvite }) => {
    const [newUser, setNewUser] = useState({ name: '', email: '', roleId: '', teamId: '', companyId: '', extension: '' });
        ...
        <Drawer isOpen={isOpen} onClose={onClose} title="Invitar Nuevo Usuario">
            <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Extensión Telefónica</label>
                        <input 
                            type="text" 
...
const UserManagementPage: React.FC = () => {
    ...
    const handleInviteUser = async (newUserData: any) => {
        try {
            ...
            const invitationData = {
                ...
                extension: newUserData.extension || null, // Add extension to invitation data if needed later
                permissions: permissions,
            };

            // NOTE: Ideally, the extension should be saved when the user registers, 
            // or updated immediately if we had a direct create method. 
            // For now, the invitation flow might need adjustment to carry 'extension' payload 
            // or we update the user doc after registration. 
            // Given current structure, let's stick to basic invite creation. 
            // In a real scenario, we'd store 'extension' in the invitation doc and apply it on registration.
    ...
    const columns = [
        ...
        {
            header: 'Ext.',
            accessor: (user: User) => <span className="font-mono text-sm text-slate-600 dark:text-slate-300">{user.extension || '-'}</span>
        },
        {
            header: 'Rol',
            accessor: (user: User) => {
